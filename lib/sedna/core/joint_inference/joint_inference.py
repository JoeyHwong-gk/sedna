...
class TSBigModelService(JobBase):
    ...
    def __init__(self, estimator=None, config=None):
        super(TSBigModelService, self).__init__(estimator=estimator, config=config)
...
class JointInference(JobBase):
   ...
   """
    ...
    def __init__(self, estimator=None, config=None):
        super(JointInference, self).__init__(estimator=estimator, config=config)
        self.job_kind = K8sResourceKind.JOINT_INFERENCE_SERVICE.value
        self.local_ip = get_host_ip()
        self.remote_ip = self.get_parameters("BIG_MODEL_BIND_IP", self.local_ip)
        self.port = int(self.get_parameters("BIG_MODEL_BIND_PORT", "5000"))
    ...
    def inference(self, data=None, post_process=None, **kwargs):
        ...
        try:
            ...
            if is_hard_example:
                cloud_result = self.cloud.inference(data.tolist(), post_process=post_process, **kwargs)
