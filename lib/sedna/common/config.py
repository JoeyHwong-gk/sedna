...
class ConfigSerializable(object):
    ...
    def from_json(cls, data):
        ...
        for attr in config:
            ...
            if isinstance(class_value, ConfigSerializable) and hasattr(config_value, 'from_json'):
...
class BaseConfig(ConfigSerializable):
    ...
    backend_type = os.getenv('BACKEND_TYPE', 'TENSORFLOW')  # ML framework backend
    lc_server = os.getenv("LC_SERVER", "http://127.0.0.1:9100")  # local control server
    # dataset
